...
/**
 * Servlet implementation class AdminServicoModInfo
 */
@WebServlet("/AdminServicoModInfo")
public class AdminServicoModInfo extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public AdminServicoModInfo() {
        super();
	...
	private List<List<String>> getPedidosUtil(){
		...
		try{
			String cmd = "SELECT * FROM utilizadores_modalidades WHERE estado=1";
	...
	private List<List<String>> getNomes(List<List<String>> pedidos){
		try{
			...
			String cmd1 = "SELECT * FROM modalidades";
			...
			List<String> listaModId = new ArrayList<String>();
			List<String> listaModNome = new ArrayList<String>();
			
			while(rs1.next()){
				listaModId.add(rs1.getString("mod_id"));
				listaModNome.add(rs1.getString("mod_nome"));
			}
			rs1.close();
			s1.close ();
			conn1.close();
			...
			for(i = 0; i < pedidos.get(1).size(); i++){
				for(j = 0; j < listaUtilId.size(); j++){
					if(pedidos.get(1).get(i).equals(listaModId.get(j))){
						pedidos.get(1).set(i, listaModNome.get(j));
						break;
					}
				}
			}
	...
	//Função que devolve uma lista de objetos [{"name":"João Rito Brandão","value":Karting},...]
